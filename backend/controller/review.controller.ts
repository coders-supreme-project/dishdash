...
export const createReview = async (req: Request, res: Response) => {
  try {
    const { review, imageUrl, userId, restaurantId } = req.body; // ✅ Changed from `reviewText` to `review`

    const newReview = await prisma.media.create({
      data: {
        review, // ✅ Corrected
        imageUrl,
...
export const updateReview = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { imageUrl, review, rating } = req.body;
  ...
  try {
    const updatedReview = await prisma.media.update({
      ...
      data: {
        imageUrl,
        review,
        rating: rating ? Number(rating) : null, // ✅ Ensure rating is a number
      },
    });
